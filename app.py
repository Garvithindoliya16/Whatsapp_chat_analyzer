import streamlit as st
import matplotlib.pyplot as plt
import seaborn as sns
import preprocessing,helper

st.sidebar.title("Whatsapp Chat Analyzer")
uploaded_file = st.sidebar.file_uploader("Choose a file")

if uploaded_file is not None:
    bytes_data = uploaded_file.getvalue()
    data=bytes_data.decode('utf-8')

    df = preprocessing.preprocess(data)



    #Fetch Unique user
    user_list=df['user'].unique().tolist()
    try:
        user_list.remove('group_notification')
    except ValueError:
        pass  # ignore if not present

    user_list.sort()
    user_list.insert(0,"Overall")

    selected_user = st.sidebar.selectbox("Show analysis wrt",user_list)

    if st.sidebar.button("Show Analysis"):
        # Stats Area
        num_messages,words,num_media_messages,num_links= helper.fetch_stats(selected_user,df)
        st.title("Top Statistics")
        col1, col2, col3, col4 = st.columns(4)

        with col1:
            st.header("Total Message")
            st.title(num_messages)

        with col2:
            st.header("Total Words")
            st.title(words)

        with col3:
            st.header("Media Shared")
            st.title(num_media_messages)

        with col4:
            st.header("Links Shared")
            st.title(num_links)

        #Monthly timeline
        st.title("Monthly Timeline")
        timeline = helper.monthly_timeline(selected_user,df)
        fig, ax=plt.subplots()
        ax.plot(timeline['time'].head(30),timeline['message'].head(30),color='black')
        ax.set_facecolor("pink")
        plt.xlabel("Time")
        plt.ylabel("Message")
        plt.xticks(rotation='vertical')
        st.pyplot(fig)

        #Daily Timeline
        st.title("Daily Timeline")
        daily_timeline = helper.daily_timeline(selected_user,df)
        fig, ax=plt.subplots()
        ax.plot(daily_timeline['only_date'],daily_timeline['message'],color='pink')
        ax.set_facecolor("black")
        plt.xlabel("Daily Timeline")
        plt.ylabel("Message")
        plt.xticks(rotation=90)
        st.pyplot(fig)

        #Activity Map
        st.title("Activity MAP")
        col1, col2 = st.columns(2)


        with col1: # Daily activity
            daily_activity=helper.activity(selected_user,df)
            st.header("Most Busy Day")
            fig, ax=plt.subplots()
            ax.bar(daily_activity.index,daily_activity.values,color='blue')
            # ax.set_facecolor("yellow")
            plt.xticks(rotation=90)
            st.pyplot(fig)

        with col2: # Monthly activity
            monthly_activity=helper.monthly_activity(selected_user,df)
            st.header("Most Busy Month")
            fig, ax=plt.subplots()
            ax.bar(monthly_activity.index,monthly_activity.values,color='orange')
            # ax.set_facecolor("yellow")
            plt.xticks(rotation=90)
            st.pyplot(fig)

        # weekly activity
        st.title("Weekly Activity Map")
        user_heatmap = helper.activity_heatmap(selected_user,df)
        fig,ax = plt.subplots()
        ax = sns.heatmap(user_heatmap)
        st.pyplot(fig)



            #Finding the busiest user in the group
        if selected_user == "Overall":
            st.title("Most Busy Users")
            x,new_df=helper.most_busy_user(df)
            col1, col2 = st.columns(2)
            fig, ax = plt.subplots()
            ax.bar(x.index, x.values, color="red")
            ax.set_xlabel("Users")
            ax.set_ylabel("Messages")
            plt.xticks(rotation=45)

            with col1:
                st.pyplot(fig)
            with col2:
                st.dataframe(new_df)

        #Wordcloud
        st.title("Wordcloud")
        df_wc=helper.create_wordcloud(selected_user,df)
        fig,ax=plt.subplots()
        ax.imshow(df_wc)
        st.pyplot(fig)

        # most common words
        most_common_df = helper.most_common_words(selected_user,df)

        fig,ax = plt.subplots()

        ax.barh(most_common_df[0],most_common_df[1],color='lightgreen')
        plt.xticks()

        st.title('Most commmon words')
        st.pyplot(fig)

        #Most used emojies
        emojies_df=helper.emojies_analysis(selected_user,df)
        if(len(emojies_df)>0):
            st.title("Emoji Analysis")
            col1,col2 = st.columns(2)

            with col1:
                st.dataframe(emojies_df)
            with col2:
                fig,ax = plt.subplots()
                plt.rcParams['font.family'] = 'Segoe UI Emoji'
                ax.pie(emojies_df[1].head(),labels=emojies_df[0].head(),autopct="%0.2f")
                st.pyplot(fig)




        

